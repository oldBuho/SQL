...
BEGIN
    ...
    SELECT
        ...
        t.unitvolume
    FROM
        TriarcLakehouse.dbo.inventsum s
            INNER JOIN
        TriarcLakehouse.dbo.inventtable t
    ...
    WHERE
        si.SiteGroup IN ('Monclova 20', 'Huehuetoca 18', 'Huehuetoca 18 Bodegas', 'Huehuetoca 97')
        AND (
            si.SiteGroup = 'Monclova 20'
            OR si.SiteGroup = 'Huehuetoca 18'
            OR si.SiteGroup = 'Huehuetoca 18 Bodegas'
            OR (si.SiteGroup = 'Huehuetoca 97' AND gs.wmslocationid IN ('VTALPG', 'VTAUSA'))
        )
    GROUP BY
        si.SiteGroupID
    )
